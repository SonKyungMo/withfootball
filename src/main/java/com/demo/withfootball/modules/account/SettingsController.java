...
import com.demo.withfootball.modules.account.validator.NicknameValidator;
import com.demo.withfootball.modules.account.validator.PasswordFormValidator;
...
public class SettingsController {
    ...
    @GetMapping(ACCOUNT)
    public String updateNicknameForm(@CurrentAccount Account account, Model model) {
        model.addAttribute(account);
    ...
    @PostMapping(ACCOUNT)
    public String updateNickname(@CurrentAccount Account account, @Valid NicknameForm nicknameForm, Errors errors,
                                 Model model, RedirectAttributes attributes) {
        if (errors.hasErrors()) {
            model.addAttribute(account);
            return SETTINGS + ACCOUNT;
    ...
    @ResponseBody
    public ResponseEntity<Zone> addZone(@CurrentAccount Account account, @RequestBody ZoneForm zoneForm) {
    ...
    @ResponseBody
    public ResponseEntity<Zone> removeZone(@CurrentAccount Account account, @RequestBody ZoneForm zoneForm) {
        Zone zone = zoneRepository.findByCityAndProvince(zoneForm.getCityName(), zoneForm.getProvinceName());
        if (zone == null) {
            return ResponseEntity.badRequest().build();
        }

...
import com.demo.withfootball.modules.event.validator.EventValidator;
...
public class EventController {
    ...
    public String newEventSubmit(@CurrentAccount Account account, @PathVariable String path,
        ...
        Event event = eventService.createEvent(modelMapper.map(eventForm, Event.class), community, account);
        return "redirect:/community/" + community.getEncodedPath() + "/events/" + event.getId();
    }
    ...
    @GetMapping("/events")
    public String viewCommunityEvents(@CurrentAccount Account account, @PathVariable String path, Model model) {
        Community community = communityService.getCommunity(path);

        model.addAttribute(account);
        model.addAttribute(community);

        List<Event> events = eventRepository.findByCommunityOrderByStartDateTime(community);
    ...
    public String cancelEvent(@CurrentAccount Account account, @PathVariable String path, @PathVariable("id") Event event) {
        ...
        return "redirect:/community/" + community.getEncodedPath() + "/events";
    }

    @PostMapping("/events/{id}/enroll")
    public String newEnrollment(@CurrentAccount Account account,
                                @PathVariable String path, @PathVariable("id") Event event) {
        Community community = communityService.getCommunityToEnroll(path);
